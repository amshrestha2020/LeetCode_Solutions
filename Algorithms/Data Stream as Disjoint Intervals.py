...
class SummaryRanges:
    ...
    def addNum(self, value: int) -> None:
        # Find the position to insert the new value while maintaining sorted order
        left, right = 0, len(self.intervals) - 1
        while left <= right:
            mid = left + (right - left) // 2
            if self.intervals[mid][0] > value:
                right = mid - 1
            elif self.intervals[mid][1] < value:
                left = mid + 1
            else:
                return  # value is already in an existing interval
        # Merge with the previous interval if necessary
        ...
        # Merge with the next interval if necessary
