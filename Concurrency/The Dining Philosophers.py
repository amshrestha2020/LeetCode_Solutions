...
class DiningPhilosophers:
    ...
    def wantsToEat(self, philosopher: int, pickLeftFork: 'Callable[[], None]', pickRightFork: 'Callable[[], None]', eat: 'Callable[[], None]', putLeftFork: 'Callable[[], None]', putRightFork: 'Callable[[], None]') -> None:
        ...
        # Ensure to always pick the lower index fork first to avoid deadlock
        if philosopher % 2 == 0:
        ...
        with self.forks[first_fork]:
            ...
            with self.forks[second_fork]:
                # Pick up the forks
                pickLeftFork() if first_fork == left_fork else pickRightFork()
                pickRightFork() if second_fork == right_fork else pickLeftFork()

                # Eat
                eat()

                # Put down the forks
                putLeftFork() if first_fork == left_fork else putRightFork()
                putRightFork() if second_fork == right_fork else putLeftFork()
