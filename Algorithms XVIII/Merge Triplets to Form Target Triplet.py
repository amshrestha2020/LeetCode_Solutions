...
class Solution:
    def mergeTriplets(self, triplets: List[List[int]], target: List[int]) -> bool:
        # Initialize the current maximum values as 0
        max_triplet = [0, 0, 0]
        
        # Filter out triplets that cannot contribute to the target
        valid_triplets = [
            triplet for triplet in triplets
            if triplet[0] <= target[0] and triplet[1] <= target[1] and triplet[2] <= target[2]
        ]
        
        # Merge valid triplets
        ...
        # Check if we reached the target triplet
