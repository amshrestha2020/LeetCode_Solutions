...
class Solution:
    def findMatrix(self, nums: List[int]) -> List[List[int]]:
        ...
        result = []  # This will hold the final 2D array
        
        # While there are still numbers to process
        while count:
            row = []  # Create a new row
            for num in list(count.keys()):  # Iterate over current unique numbers
                row.append(num)  # Add to the current row
                count[num] -= 1  # Decrease the count
                if count[num] == 0:  # Remove the number if its count is zero
                    del count[num]
